...
// -----------------------
inline unsigned long long splitBy3(int a){
	unsigned long long x = a & 0x1fffff;
...
inline unsigned long long morton3d(unsigned int x, unsigned int y, unsigned int z){
	unsigned long long answer = 0;
	answer |= splitBy3(x) | splitBy3(y) << 1 | splitBy3(z) << 2;
	return answer;
}

inline int popcnt(register unsigned int u)
{
...
//printf("niveles:%d\n",nlevel);
sprintf(n,"sib-%x.3do",in[from].m>>24);
fi=fopen(n,"w+b");
...
unsigned int sumadir=0;
unsigned int actdir=0;
...
for (i=nlevel;i>1;i--) {
	for (j=level[i-2];j<level[i-1];j++) {
		...
		vv=((sumadir-(actdir+1))<<8)|(memoct[j].c&0xff);fwrite(&vv,sizeof(int),1,fi); // 24bits direcciones
		sumadir+=popcnt((memoct[j].c&0xff));
		actdir++;
		}
	}
...
vv=0x3d000000;fwrite(&vv,sizeof(int),1,fi); // magic vv=0x3d000100
fwrite(&nodos,sizeof(int),1,fi); // cantidad de nodos
vv=0x0;fwrite(&vv,sizeof(int),1,fi);
vv=0x0;fwrite(&vv,sizeof(int),1,fi);
vv=0x0;fwrite(&vv,sizeof(int),1,fi);
...
int main(int argc, char *argv[])
{
	...
	//memoct=malloc(sizeof(struct Info)*(cnt+cnt/2));
	long long i;
